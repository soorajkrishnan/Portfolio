...
export default function SkillsAndCertifications() {
  ...
  return (
    <section id="skills" className="py-20 bg-[#1A1F2E] relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-16">
          <h3 className="font-mono text-xs text-[#D4A853] uppercase tracking-wider font-bold mb-6 text-center sm:text-left flex items-center justify-center sm:justify-start">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {certifications.map((cert) => (
              <motion.div
                ...
                className={`p-6 rounded-2xl bg-[#0F1419] border border-gray-800 hover:border-[#D4A853]/60 cursor-pointer transition-all duration-300 shadow-lg relative overflow-hidden ${
                  selectedCert === cert.id ? 'ring-2 ring-[#D4A853]' : ''
                }`}
              >
                <div className="flex items-center justify-between mb-4">
                  <div className="p-2.5 rounded-xl bg-[#1A1F2E] border border-gray-700">
                ...
                <h4 className="font-display font-bold text-lg text-[#F5F0E8]">
                  {cert.title}
                </h4>
                ...
                <p className="mt-3 text-xs text-gray-300 font-body leading-relaxed">
                  {cert.description}
                </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {skillCategories.map((cat, idx) => (
            <motion.div
              ...
              className="bg-[#0F1419] border border-gray-800 rounded-2xl p-6 hover:border-[#D4A853]/40 transition-colors"
            >
              <div className="flex items-center space-x-3 mb-4 pb-3 border-b border-gray-800">
                <div className="p-2 rounded-xl bg-[#1A1F2E] border border-gray-700">
                  {cat.icon}
                </div>
                <h3 className="font-display font-bold text-lg text-[#F5F0E8]">
                  {cat.title}
                </h3>
              </div>
              ...
              <div className="space-y-3">
                {cat.skills.map((s) => (
                  <div key={s.name} className="p-2.5 rounded-lg bg-[#1A1F2E]/60 border border-gray-800/80">
                    <div className="flex items-center justify-between text-xs font-mono font-bold">
                      <span className="text-[#F5F0E8]">{s.name}</span>
                      <span className="text-[#D4A853] text-[10px] bg-[#D4A853]/10 px-1.5 py-0.5 rounded border border-[#D4A853]/20">
                        {s.level}
                      </span>
                    </div>
